...
''' DataTypesTest.py contains all of the necessary classes and functions which pertain to the testing
of what data types are supported by a target parseLab generator module '''
...
class DataTypesTest(ParselabTest):
    ''' DataTypesTest is a class meant to test a target parseLab generator module's ability to generate
    a parser for all of the data types which are available in the parseLab framework. '''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.protocol_dir = os.path.join(self.__get_test_dir(), 'test_protocol')
        self.testcase = os.path.join(self.protocol_dir, gen_util.testcase_dirname, 'data_types_test')
    ...
    def __get_test_dir(self):
        ...
        framework '''
        return os.path.join(gen_util.PARSELAB_TOP, gen_util.unit_tests_dirname, 'DataTypesTest')
